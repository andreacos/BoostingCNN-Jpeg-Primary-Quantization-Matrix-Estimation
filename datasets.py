...
from utils import zigzag_order_arr, read_raw
import random
...
def make_dataset(in_dir, out_dir, csv_file, bsize, file_ext='*', rgb=True, jpeg_qfs=(80, 100), blocks_per_image=1000,
                 max_blocks=1e6, force_jpeg_aligned=False, force_jpeg_misaligned=False):
    ...
    with open(csv_companion, 'w') as csv:
        ...
        for idx, fim in enumerate(images):
            ...
            try:
                # Read uncompressed image: RAW case (convert RGB to BGR since following code is Opencv)
                if file_ext == '.NEF':
                    im = read_raw(fim)
                    im = im[::-1]
                else:
                ...
                if force_jpeg_aligned:
                    shift_r = 0
                    shift_c = 0
                elif force_jpeg_misaligned:
                    shift_r = random.randint(1, 7)
                    shift_c = random.randint(1, 7)
                else:
    ...
    for cnt, qf_pair in enumerate(cfg.q_factors):
        ...
        if cfg.make_train:
            ...
            make_dataset(in_dir=cfg.input_train_dir, out_dir=cfg.out_train_dir,
                         ...
                         force_jpeg_aligned=cfg.force_jpeg_aligned, force_jpeg_misaligned=cfg.force_jpeg_misaligned)
