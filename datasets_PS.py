...
def make_dataset(in_dir, out_dir, csv_file, bsize, file_ext='*', rgb=True, jpeg_qfs=(80, 100), blocks_per_image=1000,
    ...
    out_folder = make_folder(root=out_dir, subfold='{}-{}'.format(jpeg_qfs[0], jpeg_qfs[1]))

    # Read images that have been compressed once with JPEG with PhotoShop
    images = glob(os.path.join(in_dir, '{}'.format(jpeg_qfs[0]), '*' + file_ext))
    print('Found {} {} images\n'.format(len(images), file_ext))
    ...
    with open(csv_companion, 'w') as csv:
        ...
        for idx, fim in enumerate(images):
            ...
            try:
                # Read PhotoShop compressed image
                single_jpeg_im = cv2.imread(fim, int(rgb))

                # Read quantization matrix (used by PhotoShop) from the file: this will be the training label
                Q = utils.find_Qmatrix_PIL(fim)[0]
                ...
                if force_jpeg_aligned:
                    ...
                    shift_r = random.randint(0, 7)
                    shift_c = random.randint(0, 7)
                ...
                for k in range(0, multiple_height, bsize[0]):
                    for l in range(0, multiple_width, bsize[1]):
                        ...
                        if n_blocks_img in perm:
                            ...
                            # Threshold by block variance
                            # if np.var(jpeg_patch) < ????:
                            #    continue
...
if __name__ == '__main__':
    ...
    for cnt, qf_pair in enumerate(cfg.q_factors):
        ...
        print('Dataset JPEG qf1 = {} (PhotoShop), qf2 = {} (OpenCV)'.format(qf_pair[0], qf_pair[1]))

        # Train set
        # if make_train:
        #
        #     make_dataset(in_dir=input_train_dir, out_dir=out_train_dir,
        #                  file_ext='.jpg', csv_file='train{}-{}.csv'.format(qf_pair[0], qf_pair[1]),
        #                  rgb=rgb, bsize=block_size, jpeg_qfs=qf_pair,
        #                  blocks_per_image=max_blocks_img, max_blocks=n_blocks_train[cnt],
        #                  force_jpeg_aligned=force_jpeg_aligned)

        # Test set
        if cfg.make_test:

            make_dataset(in_dir=cfg.input_test_dir, out_dir=cfg.out_test_dir,
                         file_ext='.jpg', csv_file='test{}-{}.csv'.format(qf_pair[0], qf_pair[1]),
                         rgb=cfg.rgb, bsize=cfg.block_size, jpeg_qfs=qf_pair,
                         blocks_per_image=20, max_blocks=cfg.n_blocks_test[cnt],
